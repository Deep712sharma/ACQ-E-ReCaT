...
sys.path.append("./error_dec/celeba")
sys.path.append(".")
sys.path.append("./mainldm")  # or wherever your first file is located
from celeb_predadd import extract_features
...
def cache_pred_cacheerr_abCov_statistic(interval, mode):
    #all_cache = torch.load("./calibration/feature_maps.pt")
    all_cache = extract_features(args)
    interval_seq, all_cali_data, all_t, all_cali_t, _ = \
            torch.load("./calibration/celeba100_cache{}_{}.pth".format(interval, mode))
    del all_cali_data, all_t, all_cali_t, _
    ...
    torch.save((Cova_list, Covb_list), f"./error_dec/celeb/pre_cacheerr_abCov_interval{interval}_list.pth")
...
def cache_pred_quanterr_abCov_statistic(weight_bit, norm, interval, mode):
    ...
    (a_list, b_list) = torch.load(f"./error_dec/celeb/pre_cacheerr_abCov_interval{interval}_list.pth")
    interval_seq, all_cali_data, all_t, all_cali_t, _ = \
            torch.load("./calibration/celeba100_cache{}_{}.pth".format(interval, mode))
    del all_cali_data, all_t, all_cali_t, _
    print(list(interval_seq))
    ...
    # Slice to 192 channels BEFORE applying normalization
    all_cache_after_cacheadd = [cache[:, :192, :, :] for cache in all_cache_after_cacheadd]
    
    if norm:
        save_path = f"./error_dec/celeb/pre_norm_quanterr_abCov_weight{weight_bit}_interval{interval}_list.pth"
        group_weight, group_bias = torch.load("./error_dec/celeb/groupnorm_norm_params.pth")
        ...
        act_delta, act_zero_point = torch.load("./error_dec/celeb/act_quantizer_norm_params_aftercacheadd.pth", map_location=torch.device('cpu'))
        weight_delta, weight_zero_point = torch.load("./error_dec/celeb/weight_quantizer_norm_params_aftercacheadd.pth", map_location=torch.device('cpu'))
        weight, bias = torch.load("./error_dec/celeb/weight_norm_params.pth", map_location=torch.device('cpu'))
        fwd_kwargs = dict(stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1)
    else:
        save_path = f"./error_dec/celeb/pre_quanterr_abCov_weight{weight_bit}_interval{interval}_list.pth"
        act_delta, act_zero_point = torch.load("./error_dec/celeb/act_quantizer_params_aftercacheadd.pth", map_location=torch.device('cpu'))
        weight_delta, weight_zero_point = torch.load("./error_dec/celeb/weight_quantizer_params_aftercacheadd.pth", map_location=torch.device('cpu'))
        weight, bias = torch.load("./error_dec/celeb/weight_params.pth", map_location=torch.device('cpu'))
